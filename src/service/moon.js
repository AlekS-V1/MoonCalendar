...
export function getMoonInfo(date = new Date()) {
  ...
  return {
    ...
    moonDay: getMoonDay(date),
    currentDayStart: getCurrentDayStart(date).toISOString(),
    nextDayStart: getNextDayStart(date).toISOString(),
...
// ---------------------------------------------
// 8. ПРИЙМАЄ ДАТУ ТА ПОВЕРТАЄ ЛИШЕ МІСЯЧНИЙ ДЕНЬ З КАЛЕДАРЯ МІСЯЧНИХ ДНІВ
// ---------------------------------------------
...
// ---------------------------------------------
// 9. ПРИЙМАЄ ДАТУ ТА ПОВЕРТАЄ ЛИШЕ МІСЯЧНИЙ ДЕНЬ З КАЛЕДАРЯ СТРИЖОК
// ---------------------------------------------

export async function getMoonDayFromStringHaircut(dateString) {
  // Перевірка формату YYYY-MM-DD
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateString)) {
    throw new Error('Невірний формат дати. Використовуйте YYYY-MM-DD');
  }
...
// ---------------------------------------------
// 10. ПРИЙМАЄ ДАТУ ТА ПОВЕРТАЄ ЛИШЕ МІСЯЧНИЙ ДЕНЬ З КАЛЕДАРЯ РИТУАЛІВ
// ---------------------------------------------

export async function getMoonDayFromStringRitual(dateString) {
  // Перевірка формату YYYY-MM-DD
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateString)) {
    throw new Error('Невірний формат дати. Використовуйте YYYY-MM-DD');
  }
